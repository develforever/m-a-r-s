...
| 15b | Code distance (ECOC) is not a zero-shot lever on random features — it *backfires*: a min-Hamming-4 / 21-attribute vocabulary (every class structurally reachable) drops zero-shot to 0.18 ± 0.08% vs the min-Hamming-1 baseline's 3.17 ± 0.90% (paired −2.98pp, 5/5 below threshold). The pre-registered 10/10 reachability prediction is falsified (6 classes still exactly 0.0%): structural reachability is necessary, not sufficient. Mechanism: ECOC corrects errors only below a per-bit error threshold; near-chance detectors on random features push each added bit past it, so longer codes are harder to hit. Isolates the bottleneck to features (lever c), not vocabulary | G2b | NEGATIVE (strong; pre-registered) | `G2b_ecoc.json` |
| 15c | Strong features are *not* the lever either — compositional zero-shot is bottlenecked by the approach, not the representation: frozen ImageNet ResNet18 (512-d) features are statistically equivalent to random features (paired −0.40 ± 0.88pp, SZUM, ratio 0.87×), with best pretrained 2.77 ± 0.15% vs the 30% line (G3−). Random reproduces G2b exactly (3.17% / 0.18%, harness valid); ECOC still backfires on strong features (attrs21 vs attrs11 −2.69pp, SIGNAL−); old structural failures {Sandal, Bag, AnkleBoot} stay 0.0%. All three levers from the G2 diagnosis exhausted (a backfires, c null) → compositional ZS from a hand dictionary + linear concept detectors is a limit of the *paradigm*, invariant to representation and code distance. Closes series G | G3 | NEGATIVE (decisive; pre-registered binary) | `G3_pretrained.json` |
...
| 35 | Scale barrier of the exchange protocol: at 100 classes / 20 tasks the collective pays −6.67 ± 0.88pp vs the sequential agent (5/5 seeds; −0.56pp at 10 classes in L2 — ~12× growth), reaching 34.02 ± 0.65 vs 40.70 ± 0.84 | Q1 | SIGNAL− (pre-registered) | `Q1_collective_horizon.json` |
| 36 | The cost is front-loaded, not late: early adoptions reach ~37% R[t][t] vs ~78% for early learning, while late adoptions match late learning and nominally beat it relative to the per-task ceiling (0.913 vs 0.870); collective forgetting is *lower* (13.0 vs 18.3pp) — adopted classes hold but start low | Q1b | measured (first measurement of this axis; observation rank) | `Q1_collective_horizon.json` |
| 37 | The early deficit is repairable with existing machinery: forget-and-readopt of the first 5 batches (first-generation payloads) recovers 61% of the barrier (+4.09 ± 0.65pp, 5/5; early tasks ~26% → ~42%), exceeding the pre-registered prediction of +2–3pp — the immature-projection hypothesis confirmed | Q2a | SIGNAL+ (pre-registered) | `Q2_early_repair.json` |
| 38 | The barrier was a dream-budget shortage, not an information shortage: raising the adoption dream budget 500→2500 (payload unchanged, 24.1 KB) yields +10.26 ± 0.49pp — 154% of the barrier — bringing the collective to 44.29 ± 0.66 (93.4% of the all-data ceiling) | Q2b | SIGNAL+ (pre-registered) | `Q2_early_repair.json` |
| 39 | Self-dream augmentation is a lever for *any* agent: augmenting each new task's material from its own first-generation statistics (500 real + 2000 dreamed per class) lifts the sequential agent by +4.66 ± 0.52pp to 45.35 ± 0.49 — the best single agent at 100 classes, 95.7% of the all-data ceiling | Q2c | SIGNAL+ (pre-registered) | `Q2c_seq_selfdream.json` |
| 40 | At symmetric dream budgets the collective is EQUIVALENT to the sequential agent: 44.29 ± 0.66 vs 45.35 ± 0.49 (paired −1.07 ± 0.64pp, below both thresholds — SZUM). The Q2 "+3.59pp collective advantage" was a budget-asymmetry artifact, resolved by the pre-registered fairness control. The I3/L2 collective-equivalence claim extends from 10 classes / 5 agents to 100 classes / 20 agents, with 95 of 100 classes learned from 24 KB messages alone | Q1–Q2c | EQUIVALENCE (decisive control) | `Q2c_seq_selfdream.json`, `Q2_early_repair.json` |
| 41 | The levers are not additive — the budget subsumes the repair: at budget 2500 early adoptions reach ~49.5% before any repair (26.4% at budget 500), readoption adds +1.6–1.9pp locally on tasks 1–5 (5/5) but +0.14 ± 0.17pp overall (SZUM vs q2b). Three systems in equivalence: collective-2500 ≈ collective-combo ≈ seq-selfdream; recommended protocol config: budget 2500 alone | Q2d | SZUM (decisive; pre-registered anti-hypothesis confirmed) | `Q2d_combo.json` |
| 42 | Heterogeneous collective — agents with *different* backbones share classes via a regularized linear feature alignment (ridge H_A→H_B) fit on K shared calibration classes, then the standard sleep payload: recovers **92% of the homogeneous ceiling** (R2b_HET 79.98 ± 2.45% vs CEILING 86.87 ± 1.09%, R-mild, fresh seeds 5–9), a small measured cost of heterogeneity (−6.89 ± 2.33pp, SIGNAL−) and a +75pp lift over the anchor-only floor. K=2 calibration classes already suffice (81.1%, best; curve mildly decreasing). Feature-space alignment succeeds where the shared word-anchor interlingua *failed* (R1b falsified: oracle decoder + zero heterogeneity still collapsed to 4% — the 50-d anchor is class-collapsed) and where orthogonal Procrustes underperforms (R2: 31%, the inter-representation map is a general linear transform, not an isometry). First representation-agnostic transfer in the protocol | R1b/R2/R2b | falsified→**SIGNAL+** (pre-registered gate ≥70% ceiling, passed at 92%) | `R1b_translate.json`, `R2_procrustes.json`, `R2b_linear.json` |
| 43 | Heterogeneous collective across *genuinely different* representations — a from-scratch random CNN on raw pixels (128-d) and a frozen ImageNet ResNet18 (512-d), different feature content *and* different dimension — still share classes via a **rectangular** ridge map H_A(D_A)→H_B(D_B), core untouched (the 512-d sender exports a stats-only payload; the receiver stays 128-d). Foundation→scratch recovers **75.7% of the receiver's own ceiling** (HET_FS_K4 42.28 ± 2.73% vs CEILING_S 55.83 ± 1.64%; +36.9pp over the anchor floor SIGNAL+ 5/5; measured cost −13.55 ± 2.34pp SIGNAL− — *larger* than R-mild's −6.9pp, as pre-registered, because no shared feature basis exists). Machinery validated: the sanity control (a rectangular map that exists by construction) recovers 93.4% (81.18 ± 1.23%). Pre-registered prediction verified: unlike R-mild (K=2 sufficed), the calibration curve *rises* with K (K2 40.4 < K4 42.3 < K6 42.5) — a harder cross-content map needs more shared probe classes. Asymmetry (observation, confounded by rectangular-vs-square map shape): scratch→reduced (square 128→128) recovers 44.6% of its ceiling with a *falling* K-curve. Metric always relative to the receiver's own regime, never a from-scratch vs foundation count | R-hard | **SIGNAL+** (pre-registered: gate ≥50% ceiling passed at 76%, SANITY ≥65% passed at 93%) | `R_hard.json` |
