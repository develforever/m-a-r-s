"""
run_M1b_balanced_dreams.py -- M1b: budzet snow staly lacznie
(DROGA_M_PLAN.md, sekcja M1b -- dopisana PRZED runem, 20.07.2026).

Dekompozycja M1: realna degradacja dlugiego horyzontu to poznie
R[t][t] -7.8pp vs sufit (5/5 par). Podejrzany policzalny: balans snow
per klase daje ~90% snow w batchu przy 95 starych klasach. M1b zmienia
JEDNO: budzet snow projekcji = 512 LACZNIE i budzet negatywow podow
= 512 LACZNIE (dzielone po starych klasach, min 4/klase).

Wariant: m1b_seq_300 (konfiguracja jak m1_seq_300).
Baza par: m1_seq_300 z results/M1_long_horizon.json (TE SAME seedy).

Kryteria (Z GORY):
  Glowne: ACC vs m1_seq_300 (pary per-seed): SYGNAL+/-, parowy, SZUM.
  Drugie: poznie R[t][t] (16-20) vs m1_seq_300 (pary).
  Obserwacje: mech% sufitu @T=20 (kierunkowo: z 85.8% ku 96.7%);
  forgetting (ryzyko: mniejsza ochrona per klase -> F w gore;
  SYGNAL- na ACC realny -- wtedy koszt horyzontu jest strukturalny).

Wymaga: results/M1_long_horizon.json, cache cech CIFAR-100.

Tryb szybki:  python src/run_M1b_balanced_dreams.py --smoke
Pelny:        python src/run_M1b_balanced_dreams.py
"""
import argparse
import json
import math
...
def main():
    ...
    print("=" * 72)
    print(f"M1b -- budzet snow staly lacznie "
    ...
    out = {"experiment": "M1b_balanced_dreams", "device": device,
    ...
    for seed in range(n_seeds):
        ...
        print(f"[CIFAR-100] m1b_seq_300 seed {seed}: "
    ...
    out["systems"]["m1b_seq_300"] = {"per_seed": per_seed, "agg": agg}
    ...
    print(f"  m1b_seq_300  : ACC {agg['class_il_ACC']['mean']*100:.2f}"
    ...
    fname = ("M1b_balanced_dreams_smoke.json" if args.smoke
             else "M1b_balanced_dreams.json")
